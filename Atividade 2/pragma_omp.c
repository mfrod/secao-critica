...
//#pragma omp for
{
    ...
    for(i=0;i<QTD_GERACOES;i++){
        ...
        printf("\n Geracao %d: %d",i+1,QtdCelulasVivas(grid,N));
    }
...
int QtdCelulasVivas(int **M,int n){
    ...
    int resultado = 0;
    int *vetor_resultado;
    int bool_integridade = 1;

    vetor_resultado = malloc(QTD_THREADS*sizeof(int));
    for(i=0;i<QTD_THREADS;i++) vetor_resultado[i] = -1;

  
#pragma omp parallel private(j,resultado_parcial,num_thread)	
{
    resultado_parcial = 0;
    for(j=0; j<QTD_THREADS; j++){
	if(vetor_resultado[j] == -1){
		num_thread = j;
		vetor_resultado[j] = 0;		
		break;		 
	}
...
#pragma omp for   
    for (i=0; i<N; i++)
        for(j=0; j<N; j++)
            resultado_parcial = resultado_parcial + M[i][j];
    
    vetor_resultado[num_thread] = resultado_parcial;	
}   
    for(i=0;i<QTD_THREADS;i++) {
	resultado = resultado + vetor_resultado[i];
	if(vetor_resultado[i] == -1) bool_integridade = 0;
    }    

    if(bool_integridade ==1) return resultado;
    else return QtdCelulasVivas(M,n);
}
